...
// Websocket serves websocket requests from web clients
func (v JobApplicationsResource) Websocket(c buffalo.Context) error {
	hub := c.Value("hub").(*notification.Hub)
	conn, err := notification.WSUpgrader.Upgrade(c.Response(), c.Request(), nil)
	if err != nil {
		return err
	}
	client := notification.NewClient(hub, conn, make(chan []byte, 256))
	client.Register()
...
// WebsocketBroadcast broadcasts a message to all connected clients
func (v JobApplicationsResource) WebsocketBroadcast(c buffalo.Context) error {
	hub := c.Value("hub").(*notification.Hub)
	conn, err := notification.WSUpgrader.Upgrade(c.Response(), c.Request(), nil)
	if err != nil {
		return err
